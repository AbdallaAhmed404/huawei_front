...
const translations = {
  en: {
    ...
    buyNow: "Buy Now",
    viewAll: "View All",
    exploreFull: "Explore the full range",
  ...
  ar: {
    ...
    buyNow: "اشتر الآن",
    viewAll: "عرض الكل",
    exploreFull: "استكشف المجموعة كاملة",
    loading: "جاري تحميل الصوتيات..."
  }
};
    ...
    <div className={`bg-white rounded-[15px] p-8 md:p-10 flex flex-col md:flex-row items-center gap-8 border border-transparent hover:border-gray-100 shadow-lg transition-all duration-300 min-h-[350px] ${lang === 'ar' ? 'md:flex-row-reverse' : ''}`}>
      <div className="w-full md:w-1/2 flex justify-center items-center">
        <img
          src={product.image}
          alt={product.name}
          className="object-contain max-h-[220px] md:max-h-[260px] w-auto transform transition-transform duration-500 group-hover:scale-105"
        />
      </div>
      ...
      <div className={`w-full md:w-1/2 flex flex-col justify-center ${lang === 'ar' ? 'text-right' : 'text-left'}`}>
        ...
        <div className={`flex items-center gap-6 mt-auto ${lang === 'ar' ? 'justify-end' : ''}`}>
          <div className={`text-black text-[14px] font-bold flex items-center gap-1 hover:underline transition-all ${lang === 'ar' ? 'flex-row-reverse' : ''}`}>
...
export default function AudioPage() {
    ...
    const fetchData = async () => {
      try {
        ...
        const audioMain = catRes.data.categories.find(
          (c: any) => c.mainCategoryName === "Audio"
        );

        setSubCategories(audioMain?.subCategories || []);
        setProducts(prodRes.data.filter((p: any) => p.category === "Audio") || []);
      } catch (error) {
  ...
  return (
    <main className="bg-[#FBFBFB] min-h-screen pb-20 font-sans">
        ...
        <div className="flex flex-wrap justify-center gap-10 md:gap-16">
          {subCategories.map((sub) => (
            <div key={sub._id} className="flex flex-col items-center gap-3 group cursor-pointer text-center max-w-[120px]">
              <div className="w-16 h-16 md:w-30 md:h-20 rounded-[15px] flex items-center justify-center p-4 group-hover:scale-110 transition-all duration-300">
      ...
      {/* 2. الأقسام الرئيسية (Dynamic Grid) */}
            ...
            <section key={sub._id} className="space-y-8">
              ...
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                {subProducts.slice(0, 2).map((p) => (
                  <AudioHorizontalCard key={p._id} product={p} t={t} lang={lang} />
                ))}
              </div>
      ...
      {/* --- بوب اب عرض منتجات الأوديو (Modal) --- */}
      {selectedCategory && (
          ...
          <div className="bg-[#FBFBFB] w-full max-w-[1200px] max-h-[90vh] rounded-[15px] overflow-hidden flex flex-col relative shadow-2xl">
            ...
            <div className={`p-8 md:p-12 overflow-y-auto ${lang === 'ar' ? 'text-right' : 'text-left'}`}>
              <div className="mb-10">
                ...
                <p className="text-gray-500">{t.exploreFull}</p>
              </div>
