...
// مكون الكارد الأفقي الموحد - محدث لبيانات الـ API ليعمل كبورتفوليو
const HorizontalCard = ({ product, t, lang }: { product: any, t: any, lang: string }) => (
  <Link href={`/product/${product._id}`} className="block group">
    <div className={`bg-white rounded-[15px] p-10 flex flex-col md:flex-row items-center gap-10 border border-transparent hover:border-gray-100 shadow-lg transition-all duration-300 min-h-[350px] ${lang === 'ar' ? 'md:flex-row-reverse' : ''}`}>
      <div className="w-full md:w-1/2 flex justify-center items-center">
        <img
          src={product.image}
          alt={product.name}
          className="object-contain max-h-[280px] w-auto transform transition-transform duration-500 group-hover:scale-105"
        />
      </div>

      <div className={`w-full md:w-1/2 flex flex-col justify-center ${lang === 'ar' ? 'text-right' : 'text-left'}`}>
        <h3 className="text-[24px] md:text-[28px] font-bold text-black mb-4 leading-tight">
          {product.name}
        </h3>
        ...
        <div className={`flex items-center gap-8 mt-auto ${lang === 'ar' ? 'justify-end' : ''}`}>
          <div className={`text-black text-[15px] font-bold hover:underline flex items-center gap-1 transition-all ${lang === 'ar' ? 'flex-row-reverse' : ''}`}>
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const wearableMain = catRes.data.categories.find(
          (c: any) => c.mainCategoryName === "Wearable"
        );

        setSubCategories(wearableMain?.subCategories || []);
        setProducts(prodRes.data.filter((p: any) => p.category === "Wearable") || []);
      } catch (error) {
    ...
    <main className="bg-[#FBFBFB] min-h-screen pb-20 font-sans">
      ...
      <section className="max-w-[1300px] mx-auto px-6 py-16">
        <div className="flex flex-wrap justify-center gap-10 md:gap-16">
          {subCategories.map((sub) => (
            <div key={sub._id} className="flex flex-col items-center gap-3 group cursor-pointer text-center max-w-[110px] pt-2">
              <div className="w-30 h-20  flex items-center justify-center p-4 group-hover:scale-110 transition-all duration-300">
      ...
      <div className="max-w-[1400px] mx-auto px-6 md:px-12 space-y-24">
        {subCategories.map((sub) => {
          ...
          return (
            <section key={sub._id} className="space-y-10">
              <h2 className={`text-[32px] md:text-[40px] font-bold text-black ${lang === 'ar' ? 'text-right' : 'text-left'}`}>{sub.name}</h2>

              <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                {subProducts.slice(0, 2).map((p) => (
                  <HorizontalCard key={p._id} product={p} t={t} lang={lang} />
                ))}
              </div>
          ...
          <div className="bg-[#FBFBFB] w-full max-w-[1200px] max-h-[90vh] rounded-[15px] overflow-hidden flex flex-col relative shadow-2xl">
            ...
            <div className={`p-8 md:p-12 overflow-y-auto ${lang === 'ar' ? 'text-right' : 'text-left'}`}>
              <div className="mb-10">
                ...
                <p className="text-gray-500">{t.fullCollection}</p>
              </div>
